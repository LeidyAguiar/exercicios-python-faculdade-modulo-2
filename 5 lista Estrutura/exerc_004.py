...
def consultar_nome(vet_nome):
    if len(vet_nome) > 0:
        ...
        if index == -1:
            ...
            print(f'Nome completo: {vet_nome[index].nome_completo} \tData de Nascimento: {vet_nome[index].data_nascimento} \tTelefone: {vet_nome[index].telefone} \tEndereço: {vet_nome[index].endereco} \tSérie: {vet_nome[index].serie_atual}')         
    else:
